...
## CursorAgentへの伝達事項
1. プログラムの改修履歴項目の追記の取り決め
    * Ver.への改修作業で取り組んだ内容を記録するための追記を行う。
    * 改修履歴の内容は、当該項目のフォーマットに沿った記述を行う。
    * 「* <以下、改修された履歴を追記する>」の箇条書きは、今後の改修作業によって追記されることを見越したものなので、項目内からの除去は行わない。
    * 「* <以下、改修された履歴を追記する>」の箇条書きのレベルに、改修履歴の内容を追記する。
    * 「* <以下、改修された履歴を追記する>」の箇条書きは、常に箇条書き内の最後尾に位置させる。
2. Git管理の取り決め
    * CursorAgentが作業を行う分岐ブランチは一つに統一する。
    * CursorAgentの分岐ブランチの名前にチャットタイトルの名称をつけない。（ブランチの名称文字数が長くなりすぎるため）
    * 管理識別文字列が必要な場合は利用してもいいが、マージ作業が完了した管理識別された分岐ブランチを残したままにしないようにする。
    * 可能であるなら「Cursor/workingbranch」くらいがちょうどいい名称規模になりそう。
    * 管理識別が必要な場合の分岐は「Cursor/workingbranch」から行って、ここにマージするのがいいかな。
## ぷよぷよステージ表示のサイズ調整
